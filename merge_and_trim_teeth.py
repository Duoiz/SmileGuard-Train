"""
Merge Crack + Tooth annotations from Dental_Teeth into yolo_dataset_merged.
Other classes (Caries, Cavity) are skipped entirely.
Only images that contain at least one Crack annotation are copied, but the
Tooth boxes in those images are also kept so YOLO doesn't treat them as background.
"""
import os
import shutil
import random
...
DENTAL_TEETH = BASE / "Dental_Teeth"
MERGED = BASE / "yolo_dataset_merged"

# ── Current merged classes (must match yolo_dataset_merged/data.yaml) ──
# Dental_Teeth data.yaml: 0=Caries, 1=Cavity, 2=Crack, 3=Tooth
# Merged data.yaml:       0=tooth,  1=caries, 2=cavity, 3=crack, 4=calculus ...

CLASSES = [
    'tooth', 'caries', 'cavity', 'crack',
    'calculus', 'gingivitis', 'hypodontia', 'mouth_ulcer',
    'tooth_discoloration', 'braces', 'misaligned_tooth',
]

# Import Crack + Tooth from Dental_Teeth (Caries and Cavity are excluded).
# Only images that have at least one Crack annotation are copied.
# Tooth boxes in those images are preserved to avoid false-background training.
TEETH_TO_MERGED = {
    2: 3,   # Crack → crack
...
# ─────────────────────────────────────────────
print("=" * 60)
print("STEP 1: Merging Dental_Teeth dataset into yolo_dataset_merged")
print("=" * 60)

total_copied = 0
for rf_split, merged_split in SPLIT_MAP.items():
    rf_img_dir = DENTAL_TEETH / rf_split / "images"
    rf_lbl_dir = DENTAL_TEETH / rf_split / "labels"
    ...
    for lbl_file in label_files:
        ...
        # Check for filename collision — prefix with "dt_" if needed
        dst_img_name = img_src.name
        dst_lbl_name = lbl_file.name
        if (merged_img_dir / dst_img_name).exists():
            dst_img_name = "dt_" + dst_img_name
            dst_lbl_name = "dt_" + dst_lbl_name

        # Remap label class IDs — keep crack + tooth lines only
        new_lines = []
        has_crack = False
        with open(lbl_file, "r") as f:
            for line in f:
                ...
                if len(parts) < 5:
                    continue
                old_cls = int(parts[0])
                if old_cls in TEETH_TO_MERGED:
                    if old_cls == 2:   # Crack
                        has_crack = True
                    parts[0] = str(TEETH_TO_MERGED[old_cls])
                    new_lines.append(" ".join(parts))

        # Only copy images that actually contain at least one crack annotation
        if not has_crack:
            continue
