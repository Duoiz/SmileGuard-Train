"""
Merge Crack + Tooth annotations from Dental_Caries into yolo_dataset_merged.
Other Dental_Caries classes (Caries, Cavity) are skipped entirely.
...
# Only import Crack (class 2 in Dental_Mates → class 3 in merged).
# All other Dental_Mates classes are intentionally excluded.
CARIES_TO_MERGED = {
    2: 3,   # Crack → crack
        ...
        with open(lbl_file, "r") as f:
            for line in f:
                ...
                if len(parts) < 5:
                    continue
                old_cls = int(parts[0])
                if old_cls in CARIES_TO_MERGED:
                    if old_cls == 2:   # Crack
                        has_crack = True
                    parts[0] = str(CARIES_TO_MERGED[old_cls])
